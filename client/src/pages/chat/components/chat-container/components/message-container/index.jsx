  ...
  useEffect(() => {
    ...
    const getChannelMessages = async () => {
      try {
        ...
        console.log("Channel Messages Response:", response.data);
        if (response.data.messages) {
          setSelectedChatMessages(response.data.messages);
        }
      } catch (error) {
              ...
              checkIfImage(message.fileUrl)
                ? (
                  <div className="cursor-pointer" onClick={() => {
                    ...
                    <img src={`${HOST}/${message.fileUrl}`} height={300} width={300} alt="Sent image" />
                  </div>)
                : (
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <span className="text-white">{message.fileUrl.split('/').pop()}</span>
                    <button
                      className="flex items-center bg-[#8417ff] text-white rounded-full p-2 hover:bg-[#741bda] transition duration-300"
